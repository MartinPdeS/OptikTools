...
def create_jones_vector_for_ellipse(ellipse_azimuth: Angle, ellipse_ellipticity: Angle) -> np.ndarray:
    """
    ...
    electric_field_x = cosine_ellipticity * cosine_azimuth - 1j * sine_ellipticity * sine_azimuth
    electric_field_y = cosine_ellipticity * sine_azimuth + 1j * sine_ellipticity * cosine_azimuth
    jones_vector = np.array([electric_field_x, electric_field_y], dtype=np.complex128)
    norm = np.linalg.norm(jones_vector)
    return jones_vector / (norm if norm != 0 else 1.0)


def compute_stokes_parameters_from_jones(jones_vector: np.ndarray) -> Tuple[float, float, float, float]:
    """
...
def compute_ellipse_parameters_from_jones(jones_vector: np.ndarray) -> Tuple[float, float]:
    """
    ...
    """
    stokes_0, stokes_1, stokes_2, stokes_3 = compute_stokes_parameters_from_jones(jones_vector)
    if stokes_0 == 0.0:
        return 0.0, 0.0
...
class PolarizationState:
    ...
    @classmethod
    def from_ellipse_parameters(cls, ellipse_azimuth: Angle, ellipse_ellipticity: Angle) -> "PolarizationState":
        return cls(create_jones_vector_for_ellipse(ellipse_azimuth, ellipse_ellipticity))
    ...
    @classmethod
    def from_stokes_parameters(cls, stokes_0: float, stokes_1: float, stokes_2: float, stokes_3: float) -> "PolarizationState":
        """
        ...
        ellipse_azimuth_radians = 0.5 * np.arctan2(normalized_s2, normalized_s1)
        ellipse_ellipticity_radians = 0.5 * np.arcsin(np.clip(normalized_s3, -1.0, 1.0))
        return cls.from_ellipse_parameters(ellipse_azimuth_radians, ellipse_ellipticity_radians)
    ...
    def apply_linear_polarizer(self, transmission_axis_angle: Angle) -> Tuple["PolarizationState", float]:
        """
        ...
        transmitted_power_fraction = float(np.abs(local_jones[0])**2)  # since ||jones||=1
        ...
        if norm == 0:
            transmitted_global = create_jones_vector_for_linear_polarization(transmission_axis_angle)
        else:
            transmitted_global = transmitted_global / norm

        return PolarizationState(transmitted_global), transmitted_power_fraction

    def apply_general_wave_plate(self, retardance: Angle, fast_axis_angle: Angle) -> "PolarizationState":
        """
        ...
        output_jones = rotation_plus @ jones_retarder @ rotation_minus @ self.jones_vector
    ...
    def __repr__(self) -> str:
        ellipse_azimuth_radians, ellipse_ellipticity_radians = self.to_polarization_ellipse_parameters()
    ...
    def plot_polarization_ellipse(self, ellipse_scale: float = 1.0) -> None:
        ...
        minor_axis = ellipse_scale * np.tan(ellipse_ellipticity_radians) if np.cos(ellipse_ellipticity_radians) != 0 else ellipse_scale

        parametric_angle = np.linspace(0.0, 2.0 * np.pi, 600)
        coordinate_x = major_axis * np.cos(parametric_angle) * np.cos(ellipse_azimuth_radians) - minor_axis * np.sin(parametric_angle) * np.sin(ellipse_azimuth_radians)
        coordinate_y = major_axis * np.cos(parametric_angle) * np.sin(ellipse_azimuth_radians) + minor_axis * np.sin(parametric_angle) * np.cos(ellipse_azimuth_radians)

        plt.figure()
        plt.plot(coordinate_x, coordinate_y, label="polarization ellipse")
        plt.gca().set_aspect("equal", adjustable="box")
        plt.xlabel("electric field Ex (arbitrary units)")
        plt.ylabel("electric field Ey (arbitrary units)")
    ...
    def plot(self) -> None:
        ...
        ax.set_ylabel("S2 / S0")
        ax.set_zlabel("S3 / S0")
        ax.set_title("Poincaré sphere position")
        ax.legend()
        # Keep axes equal-ish
        lim = 1.1
        ax.set_xlim([-lim, lim])
